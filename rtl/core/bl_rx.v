...
module bl_rx (
  ...
  wire [15:0] newwin = {win[14:0], lb};
  wire [47:0] newhdr = {hdrsh[46:0], lb};

  always @(posedge clk) begin
    if (!rst_n) begin
      state<=S_SEARCH; win<=16'd0; hdrsh<=48'd0; hdrcnt<=6'd0;
      byte_idx<=16'd0; bitc<=3'd0; cur_byte<=8'd0; crc_iv<=1'b0;
      sfd<=1'b0; crc_ok<=1'b0; rx_signal<=8'd0; rx_service<=8'd0; rx_length<=16'd0;
      byte_valid<=1'b0; byte_data<=8'd0; done<=1'b0;
    end else begin
      sfd<=1'b0; byte_valid<=1'b0; done<=1'b0; crc_iv<=1'b0;
      if (lv) begin
        case (state)
          S_SEARCH: begin
            win <= newwin;
      ...
      if (state == S_DONE) begin
        done<=1'b1; state<=S_SEARCH; win<=16'd0;
      end
    end
  end
endmodule
